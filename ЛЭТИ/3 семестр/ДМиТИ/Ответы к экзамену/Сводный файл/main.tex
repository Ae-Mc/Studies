...
\noindent $a_0, b_0 \in \mathbb{N}$\\
$(a, b) = (a_0, b_0)4704x, y, u, v) = (1, 0, 0, 1)$\\
while $b \ne 0$\\
\indent $q = a / b$\\
\indent $(a, b) = (b, a - qb)$\\
...
$(x - qu)a_0 + (y - qv)b_0 = xa_0 + yb_0 - q(ua_0 + vb_0) = a - qb$
\subsubsection{Пример}
$\gcd(4704, 2808) = 24 = \underline{40}\cdot 4704 - \underline{67} \cdot 2808$

\begin{tabular}{c|cc|c}
    $a=4704$        & $x=1$                          & $y = 0$                           &                        \\
    $b = 2808$      & $u=0$                          & $v = 1$                           & $q = 4704 // 2808 = 1$ \\
    $a \% b = 1896$ & $u=x - qu = 1$                 & $v = y - qv = -1$                 & $q = 2808 // 1896 = 1$ \\
    $a \% b = 912$  & $u=x - qu = 0 - 1\cdot 1 = -1$ & $v = y - qv = 1 - 1\cdot(-1) = 2$ & $q = 1896 // 912 = 2$  \\
    $72$            & $3$                            & $-5$                              & $12$                   \\
    $48$            & $-37$                          & $62$                              & $1$                    \\
    $24$            & $\underline{40}$               & $\underline{-67}$                 & $2$                    \\
    $0$             &                                &                                   &                        \\
\end{tabular}
...
$\sum\limits_{i=1}^s 2^{q-l_i} \leq 2^q$
    \subsubsection{Теорема}
$p_1,\dots, p_n$ - вероятность, с которой встречаются $a_1, \dots, a_n$

    Средняя длина кодового слова $L = \sum\limits_{i=1}^n l_ip_i $ ; $L \geq H(p_1, \dots, p_s)$, где $H$ — энтропия
    \subsubsection{Доказательство}
$\sqsupset q_i=\dfrac{2^{-l_i}}{\sum\limits_{i=1}^n2^{l_i}}$ ; $L-H(p_1,\dots, p_n)=\sum\limits_{i=1}^nl_ip_i+\sum\limits_{i=1}^np_i\log\limits_2{p_i}=\sum\limits_{i=1}^np_i\log\limits_2(p_i2^{l_i})=$

$=\sum\limits_{i=1}^np_ilog{\dfrac{p_i}{2^{-l_i}}} \geq \sum\limits_{i=1}^np_i\log\limits_2{\dfrac{p_i\sum\limits_{j=1}^n2^{-l_j}}{2^{-l_i}}}=\sum\limits_{i=1}^np_i\log\limits_2{\dfrac{p_i}{q_i}}=D(p||q) \geq 0$

$\rightarrow L \geq H(p_1, \dots, p_n) + D(p||q)$

$D$ — относительная энтропия
...
$S_4(a^4) = a^2$\par
    ...
    \begin{gather}
        ...
        \end{pmatrix}
    \end{gather}
    Где $\alpha^s, \alpha^{s + 1}, \dots, \alpha^{s + 2t - 1}$ — корни $G(x)$, а $t$ — количество исправляемых ошибок
    Где $s=1, t=2$, подставим значения в матрицу и получим:
    \begin{gather}
        \begin{pmatrix}
            S_{1} & S_{2} \\
            S_{2} & S_{3}
    ...
    Если система не имеет решений (определитель матрицы коэффициентов $\begin{vmatrix}
    S_1 & S_2 \\
    S_2 & S_3 \\
\end{vmatrix}$ равен $0$), то уменьшаем $t$ на $1$ и пробуем снова.
